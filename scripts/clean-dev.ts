...
const targets = [
  ...
  'src/components/core/charts/map-chart',
...
`
    await fs.writeFile(path.join(modulesPath, 'index.scss'), indexContent, 'utf-8')
...
async function updateMenuApi() {
  const apiPath = path.resolve(process.cwd(), 'src/api/admin.ts')
