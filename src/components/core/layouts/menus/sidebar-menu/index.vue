    ...
    >
      ...
      >
        ...
        <div class="stat-total">
          <div class="stat-label">数据总览</div>
          <div class="stat-value">{{ noteCount + commentCount }}</div>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  import { useWindowSize, useTimeoutFn, useIntervalFn } from '@vueuse/core'
  import { searchNotes } from '@/api/automation/note'
  import { searchComments } from '@/api/automation/comment'
  ...
  const fetchCounts = async () => {
    try {
      const noteRes = await searchNotes({ page: 1, page_size: 1 })
      noteCount.value = noteRes.total || 0
      
      const commentRes = await searchComments({ page: 1, page_size: 1 })
      commentCount.value = commentRes.pagination?.total || 0
    } catch (e) {
  ...
  useIntervalFn(() => {
    if (menuOpen.value && !isDualMenu.value) {
      fetchCounts()
    }
  }, 5000)
  ...
  onMounted(() => {
    fetchCounts()
    mittBus.on('refresh-stats', fetchCounts)
  })

  onUnmounted(() => {
    mittBus.off('refresh-stats', fetchCounts)
  })
</script>
