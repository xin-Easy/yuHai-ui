...
export function useSettingsHandlers() {
  ...
  const basicHandlers = {
    ...
    // 菜单展开宽度
    menuOpenWidth: createValueHandler<number>((value) => settingStore.setMenuOpenWidth(value)),

    // 标签页样式
    tabStyle: createValueHandler<string>((value) => settingStore.setTabStyle(value)),

    // 页面切换动画
    pageTransition: createValueHandler<string>((value) => settingStore.setPageTransition(value)),

    // 圆角大小
    customRadius: createValueHandler<string>((value) => settingStore.setCustomRadius(value))
  }
