...
<template>
  <div class="flex-cb max-md:!block" id="art-table-header">
    ...
    <div class="flex-c md:justify-end max-md:mt-3 max-sm:!hidden">
      ...
      >
        <SvgIcon icon="ri:search-line" :class="showSearchBar ? 'text-white' : 'text-g-700'" />
      </div>
      <div
      ...
      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        <div class="button">
          <SvgIcon icon="ri:arrow-up-down-fill" />
        </div>
        <template #dropdown>
          <ElDropdownMenu>
            <div
      ...
      <div v-if="shouldShow('fullscreen')" class="button" @click="toggleFullScreen">
        <SvgIcon :icon="isFullScreen ? 'ri:fullscreen-exit-line' : 'ri:fullscreen-line'" />
      </div>
