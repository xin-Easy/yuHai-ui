...
export function useChart(options: UseChartOptions = {}) {
  const { initOptions, initDelay = 0, threshold = 0.1, autoTheme = true, onChartInit } = options
          ...
          if (entry.isIntersecting && pendingOptions && !isDestroyed) {
            ...
            requestAnimationFrame(() => {
              if (!isDestroyed && pendingOptions) {
                try {
                  // 元素变为可见，初始化图表
                  performChartInit(pendingOptions)
  ...
  const performChartInit = (options: EChartsOption) => {
    if (!chart && chartRef.value && !isDestroyed) {
      ...
      // Call onChartInit callback
      if (onChartInit && chart) {
        onChartInit(chart as any)
      }
      // 图表创建后立即设置监听器
      setupMenuWatchers()
      setupThemeWatcher()
